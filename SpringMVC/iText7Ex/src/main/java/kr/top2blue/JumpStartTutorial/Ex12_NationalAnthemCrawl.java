...
public class Ex12_NationalAnthemCrawl {
	static String destFileName = "JumpStartTutorial/result/Ex12_NationalAnthemCraw.pdf";
	static String DATA = "src/main/resources/data/NationalAnthem3.txt";
	...
	private static void makePdf(String fileName) {
		...
		try {
	        ...
	        float yOffset = -40;
	        canvas.beginText()
	            .setFontAndSize(PdfFontFactory.createFont(fontFile, PdfEncodings.IDENTITY_H), 1)
	            .setColor(yellowColor, true);
	        for (int j = 0; j < text.size(); j++) {
	            String line = text.get(j);
	            float xOffset = pageSize.getWidth() / 2 - 45 - 8 * j;
	            float fontSizeCoeff = 1 + j;
